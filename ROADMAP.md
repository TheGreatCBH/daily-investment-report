...
## 11. [x] 日内走势图重做（颜色绑当日涨跌 + 前收基准线 + 时段压缩 + 分段底纹）✓ 2026-07-20
...
- [x] 一轮 `/code-review`（high，8 角度）+ 用户最终眼验通过 + commit：出 6 finding，真实数据核验后保留 2 真实修复（#4 刻度限定当日 / #5 full_n 只取历史日）、回退 4 误判（#1/#2/#3/#6，原代码正确，成因写进注释防再改回）commit `23025e5` ✓ 2026-07-20
- [x] 午休按固定时钟窗口剔除数据源 forward-fill 假 bar：yfinance 对平安午休(11:30–13:00)每 5min 补 flat 假 bar、茅台留空 → 原「按缺口判午休」失效；`_build_session_intraday` 加 `lunch` 参数（A股 11:30–13:00 / 港股 12:00–13:00）剔除午休开区间假 bar，恢复缺口交 chart 压缩。commit `ef00cd9`，测试 44 passed ✓ 2026-07-20
